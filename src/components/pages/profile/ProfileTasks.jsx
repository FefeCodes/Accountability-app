...
export default function ProfileTasks({ userData, onUpdate, loading }) {
  ...
  const handleToggleTask = (taskId) => {
    if (isEditing) {
      const updated = draftTasks.map((task) =>
        task.id === taskId
          ? {
              ...task,
  ...
  const handleRemoveTask = (taskId) => {
    if (isEditing) {
      const updated = draftTasks.filter((task) => task.id !== taskId);
      setDraftTasks(updated);
    } else {
  ...
  const beginEdit = () => {
    setDraftTasks(JSON.parse(JSON.stringify(tasks)));
    setIsEditing(true);
  };
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="space-y-3">
        {(isEditing ? draftTasks : tasks).length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            <p>No tasks {isEditing ? "in draft" : "added"} yet.</p>
          </div>
        ) : (
          (isEditing ? draftTasks : tasks).map((task) => (
            <div
              key={task.id}
